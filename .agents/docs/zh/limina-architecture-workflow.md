...
生成命令测试必须区分启动身份与路径文本：安装后的 Limina 入口可能位于 `node_modules/.pnpm` 下，workspace 或 Node 路径也可能包含 `pnpm`。禁止该子串会误判直接 Node 调用。应比较命令 tokens，以及 shell probe 的 canonical Node 可执行文件和安装后的 CLI 入口，同时保留精确 argv/cwd 与真实 shell 回放检查。[命令测试](../../../packages/limina/src/__tests__/standalone-invocation-command.spec.ts) 覆盖包含 pnpm 的路径；[打包消费者 smoke](../../../packages/limina/smoke/generated-command.spec.ts) 覆盖安装入口身份。本地 POSIX 回放不能证明 Windows cmd.exe 或 PowerShell 成功；后者需要 [Windows CI jobs](../../../.github/workflows/ci.yml) 验证。

通过 `execFile('cmd.exe', ...)` 回放已渲染的命令时，使用 `/d /v:off /s /c`，给完整命令体包一对最外层引号，并设置 `windowsVerbatimArguments: true`。`/s /c` 解析器会去掉最外层引号；Node 默认的 Windows argv 转义反而会在已渲染的内部引号前插入反斜杠。[单包回放测试](../../../packages/limina/src/__tests__/single-package-cli.spec.ts) 与[打包消费者 shell 回放](../../../packages/limina/smoke/generated-command.spec.ts) 使用相同的传递方式。这属于测试启动逻辑，不改变生成命令的语法或查询语义。平台验证仍以原生 Windows 执行为边界。
