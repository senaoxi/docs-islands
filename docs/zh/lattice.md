...
- 希望在 CI 中证明所有源码文件都被 checker routes 或 allowlist 覆盖。
...
`config.checkers` 是 TypeScript 和 UI 框架检查能力的唯一入口。没有 `routes` 的 checker 会被忽略；`routes: {}` 是非法配置。`routes.typecheck` 参与 `lattice checker typecheck` / `checker:typecheck`；`routes.build` 参与 `lattice checker build` / `checker:build`。
...
allowlist 是所有已配置 checker routes 都无法覆盖某个源码文件后的最后手段。每个条目都必须解释为什么安全。建议在 code review 中严格审查新增 allowlist。
...
- 内置任务：`graph:check`、`proof:check`、`checker:typecheck`、`checker:build`、`package:check`；
...
运行所有已配置的 checker `typecheck` routes。`config.checkers` 中的 route registry 是唯一目标来源。
...
`lattice checker build` 会运行已配置的 checker `build` routes。
...
3. `proof:check`；
4. `checker:typecheck`；
5. 需要 build-mode 校验时运行 `checker:build`。
...
### `lattice checker typecheck` 如何选择目标？

`lattice checker typecheck` 会加载 `lattice.config.mjs`，并运行所有声明了 `routes.typecheck` 的 active checker。临时 TypeScript 项目也应该显式加入 `config.checkers.<name>.routes`，不再通过 CLI override 传入。
...
通常不建议直接放进普通 `tsc -b` graph。推荐把 Vue/VitePress/SFC 项目放到 `config.checkers.<name>.routes`，使用 `vue-tsc` preset，再由 `checker:typecheck` 和 `checker:build` 调度对应 routes。
