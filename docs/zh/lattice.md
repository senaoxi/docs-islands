...
Lattice 会生成 `tsconfig.dts.paths.generated.json`，并提示你把它加入相关声明叶子的 `extends` 第一项：
...
Checker entry 用来覆盖由 TypeScript 或框架感知工具验证的文件。典型例子是 Vue SFC、Svelte 组件、VitePress docs、主题项目、特殊 fixture 项目等。check-only 项目仍然需要补 `tsconfig*.dts.json` leaf，让 Lattice 能证明覆盖并推导本地 companion；这些 leaf 不要求承担发布产物构建职责。
...
`lattice checker build` 会对支持 build 模式的已配置 checker entry 执行 build。
...
`lattice checker typecheck` 会加载 `lattice.config.mjs`，遍历每个已配置 checker 的 `entry`，发现可达的 `tsconfig*.dts.json` leaf，把每个 leaf 映射到配对的本地 companion，然后对这些 companion 执行该 checker 的 no-emit/typecheck 模式。
