...
- 希望在 CI 中证明所有源码文件都被 graph、sidecar typecheck 或 allowlist 覆盖。
...
`graph` 是 build graph 的入口。`typecheck` 是 IDE/typecheck route 的入口。Lattice 的 graph 和 proof 检查会同时使用这两条路线判断 build config、local config 与源码覆盖关系。
...
`proof:check` 会扫描这组 source boundary，要求每个文件被 graph、sidecar target 或 allowlist 覆盖。JSON 是否纳入取决于你的仓库是否需要证明 JSON 示例/配置被覆盖。
...
sidecar target 表示“这些文件不进入 `tsc -b` graph，但仍然由其他类型检查工具覆盖”。典型例子是 Vue SFC、VitePress docs、主题项目、特殊 fixture 项目等。
...
- 内置任务：`graph:check`、`proof:check`、`tsc:run`、`package:check`；
...
- 某个源码文件既不在 graph 中，也不在 sidecar/allowlist 中。
...
发现普通 `tsconfig*.json` 目标并运行 `tsc --noEmit`。
...
`lattice tsc` 不加载 `lattice.config.mjs`。它只负责从当前 cwd 或 `-p` 指定位置发现普通 typecheck target，并拒绝 build graph config。
...
- 为什么这个文件不适合纳入 graph？
- 它由哪个 sidecar、构建步骤或运行时机制覆盖？
- 如果它失效，CI 会在哪里失败？

## 常见问题

### 为什么 `lattice tsc` 不加载 `lattice.config.mjs`？

`lattice tsc` 的定位是普通 TypeScript typecheck target runner。它从 cwd 或 `-p` 指定位置发现普通 tsconfig，并运行 `tsc --noEmit`。这样它可以作为 package-local 简单脚本使用，不依赖根治理配置。
...
通常不建议直接放进普通 `tsc -b` graph。推荐把 Vue/VitePress/SFC 项目放到 `proof.sidecarTargets`，并在 pipeline 中显式运行 `vue-tsc`。
...
- **sidecar target**：不进入 build graph，但由 `vue-tsc` 等工具覆盖的类型检查目标。
