...
至少解析出一条规则的配置处于**规则模式**——`levels` 变成规则中 `'inherit'` 的默认值，未命中的日志直接被丢弃，并且 `logger.debug()` 始终被屏蔽（即便 `debug: true` 也一样）。详见 [Runtime 配置 — Debug 模式](./runtime-config.md#debug-模式)。
:::
...
| 取值    | 含义                                                 |
| ------- | ---------------------------------------------------- |
| `'off'` | 在解析阶段删除该 preset 规则——不会产生解析后的规则。 |
| 对象    | 启用并微调该规则（可覆盖范围见下）。                 |

一个对象能覆盖多少字段，取决于该 preset 规则是怎么被启用的：

- **先经 `extends` 导入，再在 `rules` 里覆盖**——只能微调 `message` 与 `levels`。模板里的 `main` 与 `group` 被锁定，改动其中任意一个都会抛出 `The user rule cannot override "<plugin>/<rule>" plugin rule's main and group fields.`。preset 作者决定一条规则*盯住什么*，使用方只决定它*有多吵*。
