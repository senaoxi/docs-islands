...
When an issue-producing standalone command fails, it prints a standalone invocation ID and a ready-to-run shell-specific query. Every variant carries the canonical absolute config path plus the effective config loader and mode, and selects the record with `check --issues --invocation <uuid>`. The POSIX query uses `pnpm --dir <workspaceRoot> exec limina`. Both Windows queries change to the workspace and invoke the current installed Limina entry through Node, avoiding package-manager command shims that would parse CMD special characters again. PowerShell uses `Set-Location -LiteralPath` and its call operator, while CMD uses `cd /d`. All variants can therefore be copied into a shell whose current directory is outside the workspace. Each invocation is stored independently under `.limina/check/invocations/`, and issue queries locate the workspace without importing or validating the Limina config.
...
`checker typecheck` runs non-build checker entries and supplemental Astro or Svelte targets derived from build-primary source configs.
