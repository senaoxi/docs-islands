...
| `knip`, `publint`, or `@arethetypeswrong/core` is not installed; skipping check                       | An enabled optional analyzer is not installed                                                               | Install the analyzer when CI requires that coverage; a skipped analyzer alone does not make the command exit non-zero                           |
