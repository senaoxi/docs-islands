...
When an issue-producing standalone command fails, it prints a standalone invocation ID and a ready-to-run shell-specific query. The query uses `pnpm --dir <workspaceRoot> exec limina`, carries the canonical absolute config path plus the effective config loader and mode, and selects the record with `check --issues --invocation <uuid>`. The `--dir` prefix resolves the workspace-local Limina installation, so the query can be copied into a shell whose current directory is outside the workspace. Each invocation is stored independently under `.limina/check/invocations/`, and issue queries locate the workspace without importing or validating the Limina config.
