...
Besides built-in tasks, pipeline steps may also be external commands. Built-in task failures make the final result fail, but they do not block later steps through the failure policy; a named pipeline only guarantees that later steps wait for earlier steps to finish. External command failure blocks the remaining steps and marks them as `skipped`.
