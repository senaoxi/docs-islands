...
The default `tsconfig.json` should also have a clear shape. Limina treats it as a solution aggregator only when the checker-resolved file list is empty and it directly declares `references`; an explicit `files: []` is the clearest way to express that intent, while `extends` and checker-supported extensions still participate in the resolved-file check. Concrete source inputs should live in leaf configs. A named config such as `tsconfig.solution.json` may satisfy TypeScript's solution semantics, but it is not a supported Limina solution entry. Ordinary source leaf configs should not hand-write `TypeScript` project references: static source edges are inferred by Limina from TypeScript declaration providers, while real engineering edges that static analysis cannot observe should be declared explicitly through `liminaOptions.implicitRefs`.
