...
`regions.exclude` requires every rule to name one governance-root kind: `workspace-package`, `package-scope`, or `pnpm-workspace`. Its config-root-relative path globs match only exact candidate root directories of that kind, never package names or descriptor paths. Excluding an activated parent does not cascade to unmatched activated descendants. The excluded region does not belong to the current owner run, so imports from governed source into it are treated as cross-boundary access. See [Regions](./config/regions.md) for the complete matching and validation rules.
