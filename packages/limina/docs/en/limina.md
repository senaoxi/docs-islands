...
- built-in tasks: `graph:check`, `source:check`, `proof:check`, `checker:typecheck`, `checker:build`, `package:check`;
- command steps: expressed as `{ type: 'command', command, args, cwd, env }`.

`limina check` runs the built-in default pipeline: `graph:check`, `source:check`, `proof:check`, and `checker:typecheck`. `limina check <pipeline>` only runs user-defined pipelines from `limina.config.mjs#pipelines`. When the requested name is missing, Limina fails with a configuration hint instead of falling back to the default pipeline.
...
Runs Limina's built-in default check pipeline.

```sh
pnpm exec limina check
```

The default pipeline runs `graph:check`, `source:check`, `proof:check`, and `checker:typecheck` in order.

### `limina check <pipeline>`

Runs a named user pipeline from `limina.config.mjs#pipelines`.
...
If `<pipeline>` is not configured, Limina reports that the pipeline instruction was not found and asks you to define it in `limina.config.mjs#pipelines`.
...
The built-in default check runs:
...
Use a custom `limina check <pipeline>` only when the repository needs extra command, build, package, or publish steps.
