...
3. 一个受管辖源码文件能归属于某个源码类型配置；
4. 每个源码类型配置都有对应的类型输出模块；
5. 跨项目源码导入可以映射成类型输出模块之间的 `references`。
