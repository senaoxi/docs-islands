...
- [源码检查](./source-checks.md)：顶层 `source.additionalEntries` / `tsconfigOwnership` / `unusedDependencies` / `unusedModules`——依赖、模块和普通 tsconfig 归属检查。
- [图规则](./graph-rules.md)：`graph.rules.<label>`、`deny.refs`、`deny.deps` 和 `allow.refs`。
