...
`compilerOptions.customConditions` 会影响 TypeScript 和 Limina 解析器在受管辖 tsconfig 域内读取包 `exports` 字段时选择哪一个分支。比如 `browser`、`node`、`source` 这些条件，通常就代表不同的运行环境或构建方式。其他解析器可能使用全局唯一条件集合，这是另一种模型。
