...
`exclude` 匹配的是相对工作区根目录的 tsconfig 路径。它和 `config.source.exclude` 分开，后者控制的是覆盖证明里的源码文件边界。没有写 `exclude` 时，auto 模式仍会扫描能发现的每个普通 `tsconfig.json`，以及 solution references 触达的每个普通源码配置。
