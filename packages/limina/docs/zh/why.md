...
Limina 会发现这种情况，并要求你选择一种明确做法：让源码 manifest 的 exports 暴露源码入口，或停止把这条边建模为源码依赖。

例如：`@acme/app` 用 `workspace:*` 依赖 `@acme/core`，但 `@acme/core` 的源码 manifest exports 指向 `./dist/index.js`。Limina 会提示这条源码依赖解析到了构建产物。修复方式是让源码 manifest 的 `exports` 指向 `./src/index.ts`，再让 build 或 packaging 流程把发布 manifest 改写到 `./index.js` 和 `./index.d.ts`。如果 `app` 本来就要消费构建产物，则改用 `link:`、`catalog:`、`file:` 或 semver，并移除跨包 project reference。
...
Limina 希望规则保持可见。它不会把策略藏在 preset 里，而是把 checker entry、graph rules、package entries、allowlist 和 pipelines 都放在 `limina.config.mjs`。

这样架构变更就是代码审查可以读到的内容，而不是 merge 后才由 CI 报出来的惊喜。

例如：要让 browser runtime 禁止访问 Node-only 包，就在对应 declaration leaf 的 `liminaOptions` 下写 `"graphRules": ["runtime-client"]`，再在 `graph.rules.runtime-client` 里写 deny rule。之后每次有人改边界，改动都会落在配置或 tsconfig 上，reviewer 可以直接讨论这条规则是否合理。
