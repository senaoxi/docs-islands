...
Limina 用来让 TypeScript monorepo 保持一致。它会检查源码依赖图、package 归属、类型检查覆盖、源码 manifest 和构建后的 package 产物是否真的描述着同一个项目。
...
- **Release checks**：打包 npm tarball 并校验发布卫生——必需的 README 与 license 文件、不发布 source map、packed manifest 一致性，以及与 npm registry 内容比对的 workspace 发布依赖一致性。
...
- **workspace 依赖仍指向 `dist`**：某个 package 用 `workspace:*` 表示源码依赖，但源码 manifest 的 exports 仍然指向构建目录。Limina 会提示这条边没有按源码消费，因此应让源码 manifest 暴露 `src` 入口，或把这条边改成 artifact dependency。
