...
Limina 用来让 TypeScript 单体仓库保持一致。它会检查源码依赖图、包归属、类型检查覆盖、工作区导出、导出的依赖事实和构建后的包产物是否真的描述着同一个项目。
...
- **PR 改了跨包导入**：`@acme/app` 新增 `import { createClient } from '@acme/core'`，但 `app` 的声明叶子没有引用 `core`。`limina check` 会在 PR 中提示缺失项目引用或缺失包依赖声明，避免代码合并后才发现构建图已经漂移。
