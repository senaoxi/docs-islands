...
Limina 用来让 TypeScript 单体仓库保持一致。它会检查源码依赖图、包归属、类型检查覆盖、工作区导出、Nx 构建边和构建后的包产物是否真的描述着同一个项目。

小项目可能只需要 `tsc --noEmit`。但大型 workspace 通常会多出很多层：

- 包之间通过 `workspace:*` 相互依赖；
...
- **Nx 检查**：让每个包的 `project.json` `dependsOn` 构建边与 `link:` 制品依赖，以及实际导入到 `dist` 的 `workspace:*` 导出保持同步。
...
- **PR 改了跨包导入**：`@acme/app` 新增 `import { createClient } from '@acme/core'`，但 `app` 的声明叶子没有引用 `core`。`limina check` 会在 PR 中提示缺失项目引用或缺失 `workspace:*` 依赖，避免代码合并后才发现构建图已经漂移。
...
- **工作区导出同时包含源码和 `dist` 入口**：`@acme/core` 的 `.` 暴露 `src`，`./runtime` 暴露 `dist`。只要 TypeScript 和 Oxc 都能解析，Limina 接受这两类入口。实际导入源码入口时要求项目引用；实际导入 `dist` 入口时，Nx `dependsOn` 应指向 core 的构建目标。

## 接入后的工作方式

接入 Limina 后，你会得到一组可以稳定放进本地、PR 和发布流程的检查：

- 本地开发时，快速知道改动有没有破坏 TypeScript 图或检查器覆盖；
- PR 中，评审者能看到架构边界变更是否写进了 `limina.config.mjs`、`package.json` 或 `tsconfig*.dts.json`；
