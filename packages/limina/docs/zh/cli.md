...
Limina 的 `CLI` 主线围绕 `TypeScript` 项目引用展开。它根据配置和源码计算工程图，再基于这份结果检查源码归属、包依赖、项目引用、检查器入口和源码覆盖。需要生成检查器文件的命令会把工程图物化到 `.limina`；只读取图事实的检查直接使用内存结果。没有自动治理时，开发者需要手动判断并维护哪些源码关系应该进入 `references` 图；Limina 的作用是把这类判断转化为可重复运行的命令和报告。
...
`graph check`、`source check`、`proof check` 这类只做验证的命令会在内存中计算生成图，不会写出检查器配置。managed `build`、`checker build`、`checker typecheck`，以及包含检查器任务或 `graph:prepare` 的 `check` 流水线，会在执行前物化所需文件。

当你只需要定位上一次失败的原因时，不必重新运行所有检查，可以读取最近一次完整检查快照；这个模式只定位工作区，不会加载或执行 Limina 配置：
...
迁移不会安装 Astro 或 Svelte 依赖，不会运行 `astro sync`，也不会改写框架源码。下一次物化生成图时，Limina 会根据迁移后的源码配置和实际文件推导框架能力。如果存在受支持的旧 `.limina/manifest.json`，Limina 只把它当作产物归属 ledger，用它删除旧生成路径，再替换为当前的 version 3 manifest；框架能力 descriptor 始终是实时图事实，不会持久化进 manifest。
...
源码中内置的显式非构建型检查器包括 `vue-tsgo` 和 `svelte-check`。`vue-tsgo` 的入口仍可参与源码图和覆盖证明；独立 `svelte-check` 入口会参与覆盖证明和类型检查执行，但当前不作为源码图提供者。二者都不作为 `checker build` 的构建型执行入口。
...
`checker typecheck` 不接受配置路径、`--preset` 或 `--watch`。这些框架目标明确不支持 watch；源码配置、解析器包、框架生成类型或框架源码变化后，需要重新运行命令。如果既没有非构建入口，也没有补充框架能力，该命令会以无可运行目标的状态通过。
...
它同样基于 `package.entries` 选择产物目录，并要求被检查的包与当前工作目录或 `--package` 选择匹配。命令会读取输出目录中的 `package.json`，检查不应出现在发布产物中的本地依赖声明，例如 `workspace:`、`link:`、`file:` 或 `catalog:`；如果输出清单标记为 `private: true`，也会作为发布前问题报告。随后它会打包产物，并执行发布一致性检查，包括 `tarball`、清单文件、注册表基线或内容哈希相关的检查，具体取决于配置和当前产物状态。启用 `release.npmPackageJsonLint` 后，还会使用工作区另行安装的 `npm-package-json-lint` 检查打包清单。
...
| `checker typecheck does not accept --preset` 或 `--watch`                               | typecheck 会运行完整的第二类目标集合，不支持逐目标框架监听 | 源码配置、解析器包、框架生成类型或框架源码变化后，重新运行 `checker typecheck`    |
