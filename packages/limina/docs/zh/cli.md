...
Knip 相关检查依赖 `knip` peer dependency，并受 `source.knip` 配置影响。关闭或调整 Knip 配置只影响这部分源码使用检查，不会关闭图检查、覆盖证明检查或检查器执行。
...
托管模式会生成并运行 `.limina/tsconfig/checkers/<checker>/outputs/` 下的输出构建配置。多个构建型 checker 同时匹配目标时，必须传 `--preset`。`--watch` 使用对应 checker adapter 的 watch 能力；不支持时会明确失败。
