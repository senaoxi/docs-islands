...
Limina 需要运行在 pnpm 工作区内。当前包配置要求 Node `^20.19.0 || >=22.12.0`。如果手动安装，使用：
...
| 对指定配置运行内部声明图构建       | `limina checker build <config>`            | 只接受 Limina 管理的源码配置或 solution 配置，不 raw build                       |
| 构建用户可消费产物                 | `limina build <config>`                    | 只接受 Limina 管理且声明了 `liminaOptions.outputs` 的源码叶子或 solution 配置    |
...
| 检查已构建包产物                   | `limina package check`                     | 已有 `package.entries[].outDir`，需要检查 manifest、publint、ATTW 或产物导入边界 |
...
Knip 相关检查依赖 `knip` peer dependency，并受 `source.knip` 配置影响。关闭或调整 Knip 配置只影响这部分源码使用检查，不会关闭图检查、证明检查或检查器执行。
...
`build` 构建用户可消费产物。托管模式只接受 Limina 管理的配置：源码叶子必须声明 `liminaOptions.outputs`；solution 配置下递归引用的源码叶子至少要有一个声明了 `liminaOptions.outputs`。
...
托管模式会生成并运行 `.limina/tsconfig/checkers/<checker>/outputs/` 下的 output build config。多个构建型 checker 同时匹配目标时，必须传 `--preset`。`--watch` 使用对应 checker adapter 的 watch 能力；不支持时会明确失败。

`--raw` 用于直接运行 `tsc`、`tsgo` 或 `vue-tsc` 构建用户维护的 tsconfig。raw 模式必须传 `--preset`，不会准备生成图，不读取 `liminaOptions.outputs`，不使用 Limina 推断引用，并拒绝 `.limina` 下的生成配置。
...
不带 `config` 时，命令使用生成工程图中的所有构建型检查器入口。带 `config` 时，Limina 只解析已管理配置对应的内部声明目标；如果配置不由 Limina 管理，会立即失败。该命令不读取 `liminaOptions.outputs`，不会生成 `dist` 等用户产物，也不会 raw build 用户维护的 tsconfig。
...
源码中内置的非构建型检查器包括 `vue-tsgo` 和 `svelte-check`。它们可以参与检查器入口和部分源码图语义，但不作为 `checker build` 的构建型执行入口。
...
它读取配置中的 `package.entries`，进入每个条目的 `outDir`，读取已构建产物中的 `package.json`。如果启用了 `publint` 或 `attw`，会先把输出目录打成临时 tarball 再交给对应工具检查；如果启用了 `boundary`，会扫描输出目录中的 JavaScript 文件，检查外部包导入、自引用导入和 Node 内置模块使用是否符合产物 manifest 与配置。
...
它同样基于 `package.entries` 选择产物目录，并要求被检查的包与当前工作目录或 `--package` 选择匹配。命令会读取输出目录中的 `package.json`，检查不应出现在发布产物中的本地依赖声明，例如 `workspace:`、`link:`、`file:` 或 `catalog:`；如果输出 manifest 标记为 `private: true`，也会作为发布前问题报告。随后它会打包产物，并执行发布一致性检查，包括 tarball、manifest、注册表基线或内容哈希相关的检查，具体取决于配置和当前产物状态。
...
| `limina build --raw requires --preset`                                     | raw 模式没有指定 checker preset              | 改为 `limina build <config> --raw --preset tsc`                                                                 |
...
| `Missing peer dependency ...`                                              | 某个检查器或包检查工具未安装                 | 按报告提示安装对应 peer dependency，例如 `typescript`、`vue-tsc`、`knip`、`publint` 或 `@arethetypeswrong/core` |
...
- `packages/limina/src/package-check/runner.ts`：实现已构建包产物的 manifest、publint、ATTW 和产物导入边界检查。
- `packages/limina/src/commands/release.ts`：实现发布前产物一致性检查，包括本地依赖声明、`private` manifest、tarball 和发布一致性问题报告。
