...
Limina 识别这些治理单元和边界后，`regions.exclude` 可以裁剪已识别的工作区包、已扩展包作用域、停止的包作用域根或嵌套工作区根。每条排除配置都必须说明原因，任意普通目录不是合法目标。被排除根及其后代都不属于当前运行，受治理源码导入其中内容时会按跨边界访问处理。完整规则见[治理区域](./config/regions.md)。
