...
Limina 的源码检查会把被检查的文件放回 `pnpm` 工作区包作用域中理解。这里的“源码归属方”指 `pnpm` 工作区包作用域，而不是任意最近的嵌套 `package.json`。一个被检查的源码文件如果找不到工作区包作用域，会被报告为问题；一个普通源码 `tsconfig*.json` 如果覆盖了多个工作区包作用域的文件，也会被报告为边界过宽。
...
这条约束的重点很简单：先让每个源码文件能回答“我属于哪个工作区包作用域”，后面的导入关系、类型图和产物检查才有稳定基础。
...
| 源码文件有清楚工作区包作用域        | 一个 `tsconfig` 混进多个包的源码                    |
...
```text
1. 这个文件属于哪个 pnpm 工作区包作用域？
2. 当前源码文件是否只归属于一个普通类型检测模块？
3. 当前 tsconfig 是否只覆盖这个工作区包作用域内的源码？
