...
`lattice.config.mjs` 必须 default export 一个配置对象、配置对象的 Promise，或接收 `{ command, mode }` 的配置函数。推荐使用 `defineConfig(...)` 获得编辑器提示和类型导出。`--mode` 会传给配置函数；省略时使用 `process.env.NODE_ENV`，再回退到 `default`。未指定 `--config` 时，Lattice 会从命令 cwd 向上查找 `lattice.config.mjs`；相对 `--config` 从命令 cwd 解析，绝对路径按原样使用。
...
package checks 会扫描配置 `outDir` 目录下的构建后 package 产物。
指定 `--package` 时，它必须精确匹配 `targets[].name`。未指定
`--package` 时，Lattice 会检查 `cwd/package.json#name`；如果该名称匹配已配置目标，
则只运行该目标，否则运行所有目标。

| 字段                                         | 说明                                                                   |
| -------------------------------------------- | ---------------------------------------------------------------------- |
| `targets[].name`                             | `--package` 使用的目标名，通常是 package name。                        |
| `targets[].outDir`                           | 包含构建产物和 `package.json` 的目录。                                 |
