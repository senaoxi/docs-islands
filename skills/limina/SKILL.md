---
name: limina
description: Use when configuring or operating the `limina` CLI in a TypeScript pnpm monorepo. Triggers on tasks involving `limina.config.mjs`, `defineConfig` from `limina`, the commands `limina init/check/graph/proof/source/checker/package/build/release`, the built-in tasks `graph:prepare`/`graph:check`/`source:check`/`proof:check`/`checker:build`/`checker:typecheck`/`package:check`, generated `.limina` checker graphs, source `tsconfig.json` entries, `tsc -b` project references, `workspace:*` source vs artifact dependencies, `publint`/`@arethetypeswrong/core`/runtime boundary checks before publishing, or source-vs-built package manifest alignment.
---
...
| ------------------------ | ------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------- |
| Source entry             | `tsconfig.json`                                         | Ordinary workspace-root-relative checker entry. Auto checker discovery starts from these files unless checkers are configured explicitly.      |
| Source project config    | `tsconfig*.json` reached from a source entry            | Owns typecheck semantics for source files. Limina follows solution references from selected entries.                                           |
| Generated checker graph  | `.limina/tsconfig/checkers/<checker>/**`                | Internal declaration/build graph generated by `graph:prepare`, graph-consuming commands, `limina check`, or `prepareGeneratedTsconfigGraph()`. |
...
Generated `.limina` files are implementation artifacts. Users should maintain source `tsconfig*.json`, package manifests, and `limina.config.mjs`; do not ask users to handwrite generated declaration graph files.
...
`limina init` searches upward for `pnpm-workspace.yaml`, confirms the workspace root, writes an auto-first `limina.config.mjs`, ensures `.limina/` is ignored, creates or updates the root `package.json`, and adds missing `limina` / `typescript` devDependencies. It does not generate `.limina` graph files; graph files are generated later by `limina graph prepare`, graph-consuming commands, `limina check`, or `prepareGeneratedTsconfigGraph()`.
...
| ------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------ |
| `limina init [--yes]`                                              | Bootstrap config, root script, ignored generated directory, dependencies, optional skill install | No pnpm workspace root, workspace package names are missing                                                                          |
