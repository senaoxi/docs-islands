...
Other named exports from the `limina` entry: `validateLiminaConfig`, `getActiveCheckers`, `getActiveCheckerExtensions`, `runInit`, `runSourceCheck`, `runCheckerBuild`, `runCheckerTypecheck`, `runBuild`, `runGraphCheck`, `runGraphPrepare`, `runGraphExport`, `prepareGeneratedTsconfigGraph`, `collectDependencyGraph`, `stringifyDependencyGraph`, `createLiminaFlowReporter`, `LiminaFlowReporter`, `isOrdinaryTypecheckConfigPath`, and the full set of config types (`LiminaConfig`, `ResolvedLiminaConfig`, etc.). For type-only usage, import types from the root entry with `import type { LiminaConfig } from 'limina'`.
